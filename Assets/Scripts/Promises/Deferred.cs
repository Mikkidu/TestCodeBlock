...
namespace PU.Promises
{
    public class Deferred : BaseDeferred, IPromise
    {
        ...
        {
            return Reject(GenerateException(reason, args));
        }

        public static Exception GenerateException(string reason, params object[] args)
        {
            try
            {
                return new Exception(string.Format(reason, args));
            }
            catch (Exception e)
            {
    ...
    public class Deferred<T> : BaseDeferred, IPromise<T>
    {
        ...
        {
            return GetFromPool().Resolve(result);
        }

        public static IPromise<T> Rejected(string reason, params object[] args)
        {
            return GetFromPool().Reject(Deferred.GenerateException(reason, args));
        }
        ...
        {
            return Reject(Deferred.GenerateException(reason, args));
        }
    ...
    public class Deferred<T1, T2> : BaseDeferred, IPromise<T1, T2>
    {
        ...
        public static IPromise<T1, T2> Rejected(Exception ex)
        {
            return GetFromPool().Reject(ex);
        }
